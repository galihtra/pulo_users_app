...
import 'package:users_app/global/trip_courier_var.dart';
import 'package:users_app/global/global_var.dart';

import '../../main/auth/auth_page.dart';
import '../../methods/common_methods.dart';
import '../../methods/manage_couriers_methods.dart';
import '../../methods/push_notification_service.dart';
import '../../models/direction_details.dart';
import '../../models/online_nearby_couriers.dart';
...
class HomePageCourier extends StatefulWidget {
  const HomePageCourier({super.key});

  @override
  State<HomePageCourier> createState() => _HomePageCourierState();
}

class _HomePageCourierState extends State<HomePageCourier> {
  ...
  bool nearbyOnlineCouriersKeysLoaded = false;
  BitmapDescriptor? carIconNearbyCourier;
  DatabaseReference? tripRequestRef;
  List<OnlineNearbyCouriers>? availableNearbyOnlineCouriersList;
  StreamSubscription<DatabaseEvent>? tripStreamSubscription;
  bool requestingDirectionDetailsInfo = false;

  makeDriverNearbyCarIcon() {
    if (carIconNearbyCourier == null) {
      ImageConfiguration configuration =
          createLocalImageConfiguration(context, size: Size(0.5, 0.5));
      BitmapDescriptor.fromAssetImage(
              configuration, "assets/images/tracking_couriers.png")
          .then((iconImage) {
        carIconNearbyCourier = iconImage;
      });
    }
  }
  ...
  updateAvailableNearbyOnlineDriversOnMap() {
    ...
    for (OnlineNearbyCouriers eachOnlineNearbyDriver
        in ManageCouriersMethods.nearbyOnlineCouriersList) {
      LatLng driverCurrentPosition = LatLng(
          eachOnlineNearbyDriver.latCourier!, eachOnlineNearbyDriver.lngCourier!);

      Marker driverMarker = Marker(
        markerId: MarkerId(
            "courier ID = " + eachOnlineNearbyDriver.uidCourier.toString()),
        position: driverCurrentPosition,
        icon: carIconNearbyCourier!,
      );
  ...
  initializeGeoFireListener() {
    Geofire.initialize("onlineCouriers");
      ...
      if (driverEvent != null) {
        ...
        switch (onlineDriverChild) {
          case Geofire.onKeyEntered:
            OnlineNearbyCouriers onlineNearbyCouriers = OnlineNearbyCouriers();
            onlineNearbyCouriers.uidCourier = driverEvent["key"];
            onlineNearbyCouriers.latCourier = driverEvent["latitude"];
            onlineNearbyCouriers.lngCourier = driverEvent["longitude"];
            ManageCouriersMethods.nearbyOnlineCouriersList.add(onlineNearbyCouriers);

            if (nearbyOnlineCouriersKeysLoaded == true) {
          ...
          case Geofire.onKeyExited:
            ManageCouriersMethods.removeCourierFromList(driverEvent["key"]);
          ...
          case Geofire.onKeyMoved:
            ...
            onlineNearbyCouriers.latCourier = driverEvent["latitude"];
            onlineNearbyCouriers.lngCourier = driverEvent["longitude"];
            ManageCouriersMethods.updateOnlineNearbyCouriersLocation(
                onlineNearbyCouriers);
          ...
          case Geofire.onGeoQueryReady:
            nearbyOnlineCouriersKeysLoaded = true;
  ...
  makeTripRequest() {
    tripRequestRef =
        FirebaseDatabase.instance.ref().child("tripRequestsCourier").push();
    ...
    tripStreamSubscription =
      ...
      if ((eventSnapshot.snapshot.value as Map)["driverName"] != null) {
        nameCourier = (eventSnapshot.snapshot.value as Map)["driverName"];
      }
      ...
      if ((eventSnapshot.snapshot.value as Map)["driverPhoto"] != null) {
        photoCourier = (eventSnapshot.snapshot.value as Map)["driverPhoto"];
      }

      if ((eventSnapshot.snapshot.value as Map)["carDetails"] != null) {
        carDetailsCourier = (eventSnapshot.snapshot.value as Map)["carDetails"];
      }
      ...
      if ((eventSnapshot.snapshot.value as Map)["riderLocation"] != null) {
        double driverLatitude = double.parse(
            (eventSnapshot.snapshot.value as Map)["riderLocation"]["latitude"]
                .toString());
        double driverLongitude = double.parse(
            (eventSnapshot.snapshot.value as Map)["riderLocation"]["longitude"]
                .toString());
      ...
      if (status == "accepted") {
        ...
        setState(() {
          markerSet.removeWhere(
              (element) => element.markerId.value.contains("courier"));
        });
      }
  ...
  searchDriver() {
    if (availableNearbyOnlineCouriersList!.length == 0) {
      cancelRideRequest();
      resetAppNow();
      noDriverAvailable();
      return;
    ...
    var currentDriver = availableNearbyOnlineCouriersList![0];

    //send notification to this currentDriver - currentDriver means selected driver
    sendNotificationToDriver(currentDriver);

    availableNearbyOnlineCouriersList!.removeAt(0);
  }

  sendNotificationToDriver(OnlineNearbyCouriers currentRider) {
    //update driver's newTripStatus - assign tripID to current driver
    DatabaseReference currentDriverRef = FirebaseDatabase.instance
        .ref()
        .child("couriers")
        .child(currentRider.uidCourier.toString())
        .child("newTripStatusCourier");
    ...
    tokenOfCurrentDriverRef.once().then((dataSnapshot) {
      ...
      var timerCountDown = Timer.periodic(oneTickPerSec, (timer) {
        requestTimeoutCourier = requestTimeoutCourier - 1;
                                ...
                                children: [
                                  ...
                                  GestureDetector(
                                    onTap: () {
                                      ...
                                      availableNearbyOnlineCouriersList =
                                          ManageCouriersMethods.nearbyOnlineCouriersList;
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        GestureDetector(
                          onTap: () {
                            launchUrl(Uri.parse("tel://$phoneNumberCourier"));
                          },
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.center,
                            children: [
